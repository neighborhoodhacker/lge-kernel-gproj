...
#if DEBUG
        pr_info("[sweep2wake]: sweep_coord,sweep_height(%4d,%4d) single:%s barrier[0]:%s barrier[1]:%s\n",
                sweep_coord, sweep_height, (single_touch) ? "true" : "false", (barrier[0]) ? "true" : "false", (barrier[1]) ? "true" : "false");
#endif
...
/********************* SYSFS INTERFACE ***********************/
static ssize_t s2w_switch_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%i\n", s2w_switch);
}

static ssize_t s2w_switch_store(struct kobject *kobj,
